...
export default function App() {
  ...
  async function buscarTarefas(){
    const resposta = await supabase
      ...
      if(resposta.error){
        alert('deu red' + resposta.error.message);
      } 
      else {
  ...
  async function adicionarTarefa(evento) {
    if(!titulo || !descricao){
      alert('por favo,preecha pelo menos o título da tarefa.');
      return;
    }

    const reposta = await supabase
       .from('tarefas')
       .insert(
        [
          {
       ...
       if(resposta.erro){
        alert('erro ao enviar os dados para a a tabela: ' + reposta.error.message);
       }
       else {
        setTitulo('');
        setDescricao('')
